...
export class WorkboardStore extends WorkboardNotificationStore {
   ...
   *
   * `openSqlite()` opens a fresh SQLite connection every call and nothing ever
   * closes it. Plugin registration runs more than once per process (reloads,
   * re-registration), so each pass leaked another connection to the same file:
   * a live Gateway was observed holding 43 handles to `workboard.sqlite` plus
   * 43 to its WAL. In WAL mode every extra connection keeps re-reading shared
   * index pages to notice other writers, which showed up as ~150k read
   * syscalls per second on an otherwise idle Gateway and as `slow SQLite
   * transaction lock wait` warnings.
   *

...
function sessionSlug(displayName: string | undefined): string {
...
function sessionKeyUuid(sessionKey: string): string | null {
  const rest = agentSessionKeyParts(sessionKey)?.rest;
...
function sessionPath(params: {
  ...
  const uuid = parsed ? sessionKeyUuid(rawKey) : null;
  if (uuid) {
    let length = 8;
    const slug = sessionSlug(params.displayName);

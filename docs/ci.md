...
- Media and UI runtime configs run as two shards (`core-runtime-media`, `core-runtime-ui`) rather than one five-config job. That job was the densest packing in the matrix; splitting it is preventive rather than a fix — it has not failed on `main` — and it reduces the crowding that resource-bound suites elsewhere are sensitive to. Both halves stay in the large-runner set, so the cost is one extra 8 vCPU job.
